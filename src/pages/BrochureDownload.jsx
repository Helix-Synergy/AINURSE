...
const BrochureDownload = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Trigger brochure download immediately or as a fallback
    const triggerDownload = () => {
      const link = document.createElement("a");
      link.href = "/AINURSE (1).pdf";
      link.setAttribute("download", "AINURSE (1).pdf");
    ...
    try {
      ...
      if (res.status === 200 || res.status === 201) {
        setStatus({ type: "success", message: "Form submitted successfully. Your download should start shortly." });
        triggerDownload();
      } else {
        // If backend fails but we want the user to have the brochure anyway
        setStatus({ type: "success", message: "Thank you! Your download should start shortly." });
        triggerDownload();
      }
    } catch (err) {
      console.error(err);
      // Fallback: trigger download anyway if the form fails
      setStatus({ type: "success", message: "Thank you! Your download should start shortly." });
      triggerDownload();
    } finally {
      setLoading(false);
